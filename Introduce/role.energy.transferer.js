...
// 			}
			...
			const this_shard_config = Memory.config.shards[Game.shard.name];
...
// 				}
			...
			if(!target && creep.memory.rerun) {
				const labs = cash.getLabs(creep.room.name).filter((l) => l.store.getFreeCapacity(RESOURCE_ENERGY) > 0);
				if(labs.length > 0) {
