    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && Nspawns) {
				...
				const Sp8 = (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn21' || spawn.name == 'Spawn23' || spawn.name == 'Spawn--'):
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W26S26') &&
						...
						if(tools.getInviderCoreLevel('W26S26') == 1) {
							if(Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 5287);
						}
					}
					else {
						if(Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[9][L], 5286);
						if(Sp11)	spawns.tryCreateCreep(spawn, WORKER[7][S+1], 5284);
						if(Sp11)	spawns.tryCreateCreep(spawn,   CARIER[7][L], 5281);
					}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S25')) {
						...
						if(Sp12)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5307);
						if(Sp12)	spawns.tryCreateCreep(spawn, WORKER[7][M], 5304);
						if(Sp12)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5301);
						if(Sp12)	spawns.tryCreateCreep(spawn, UPGRADER[M], 5305);
					}
            ...
            if(Sp5)	spawns.tryCreateCreep(spawn, CARIER[7][M], 551);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S54') &&
						...
						if(tools.getInviderCoreLevel('W56S54') == 1) {
							if(Sp5)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 557);
						}
					}
					else {
						if(Sp5)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 556);
						if(Sp5)	spawns.tryCreateCreep(spawn,  WORKER[7][S+1], 554);
            if(Sp5)	spawns.tryCreateCreep(spawn,  CARIER[7][L], 551);
					}
            ...
            if(Sp7)	spawns.tryCreateCreep(spawn,  CARIER[7][L], 621);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W55S54') &&
						...
						if(tools.getInviderCoreLevel('W55S54') == 1) {
							if(Sp7)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 637);
							if(Sp5)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 637);
						}
					}
					else {
						if(Sp5)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 636);
						if(Sp7)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 636);
