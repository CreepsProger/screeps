...
var spawns = {
   ...
   work_efficiency: function(type,range) {
    ...
    tryCreateCreep: function(spawn, type, weight, modification = flags.getModification(weight), boosts = undefined, needed = flags.getNeeded(weight), max_needed = flags.getMaxNeeded(weight, needed)) {
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && Nspawns) {
				...
				const Sp11= (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):
				         		(Game.shard.name == 'shard1')? (All || spawn.name == 'Spawn29' || spawn.name == 'Spawn32' || spawn.name == 'Spawn--'):
				            (Game.shard.name == 'shard3')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):All;
        ...
        if(Game.shard.name == 'shard1') {
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S26')) {
						if(Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5146);
					}
					else {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S26')) {
						if(Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5316);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W28S26')) {
							if(Sp11)	spawns.tryCreateCreep(spawn, CLAIMER[7][M], 5310);
						}
						if(Sp11)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5314);
          ...
          if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S51')) {
            ...
            if(Sp5)	spawns.tryCreateCreep(spawn,  CARIER[7][L], 561);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W55S53')) {
						if(Sp4)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 576);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W55S52')) {
              if(Sp4)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 570);
						}
						if(Sp4)	spawns.tryCreateCreep(spawn, WORKER[5][M], 574);
