	...
	run: function() {
		...
		for(var name in Game.spawns) {
			...
			if(!spawn.spawning) {
				...
				if(CL >= 4) spawns.tryCreateCreep(spawn,        90909, 50, N<(config.maxCreepsNumber+1)? 9:0); // V 1-1 E  1800   Worker
				if(CL >= 4) spawns.tryCreateCreep(spawn,         1709, 40, N<(config.maxCreepsNumber+1)? 3:0); // V 1-1 E   1300   Worker
