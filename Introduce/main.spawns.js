			...
			if(!spawn.spawning && spawn.name != 'Spawn19' && Nspawns) {
				...
				if(false) {
					...
					if(conditions.TO_SPAWN_MAIN_ROOMS()) {
						...
						if(spawn.name == 'Spawn2' || spawn.name == 'Spawn3' || !!rerun)
						 																			spawns.tryCreateCreep(spawn,             2412, 51, 1, 2); // V 1-1 E    Carier
					}
					if(conditions.TO_SPAWN_CLAIMING_ROOMS()) {
						...
						if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S34')) {
							...
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S35')) {
								...
								if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,           140711,124, 1, 1); // V 1-1 E    Worker
								if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,             3015,121, 1, 2); // V 1-1 E    Carier
								...
								if(conditions.TO_SPAWN_CLAIMING_ROOMS3()) {
									if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S35')) {
										...
										if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,    2000000000002,130, 1, 2); // V 1-1 E   Claimer
										if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,            90909,134, 1, 1); // V 1-1 E    Worker
										if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,             1608,131, 1, 1); // V 1-1 E    Carier
									}
