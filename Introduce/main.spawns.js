	...
	run: function() {
		...
		for(var name in Game.spawns) {
			...
			if(!spawn.spawning) {
				...
				if(CL >= 4) spawns.tryCreateCreep(spawn,        60606, 50, N<(config.maxCreepsNumber+1)? 8:0); // V 1-1 E  1200   Worker
				if(CL >= 4) spawns.tryCreateCreep(spawn,        60606, 40, N<(config.maxCreepsNumber+1)? 5:0); // V 1-1 E  1200   Worker
