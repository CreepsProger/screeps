    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && Nspawns) {
				...
				const Sp7 = (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn18' || spawn.name == 'Spawn20' || spawn.name == 'Spawn--'):
				...
				const Sp12= (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):
				         		(Game.shard.name == 'shard1')? (All || spawn.name == 'Spawn33' || spawn.name == 'Spawn35' || spawn.name == 'Spawn--'):
				            (Game.shard.name == 'shard3')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):All;
        ...
        if(Game.shard.name == 'shard1') {
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S27')) {
						if(Sp8)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5106);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S27')) {
							if(Sp8)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 5100);
						}
						if(Sp8)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5104);
						if(Sp8)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5101);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S28')) {
						if(Sp8)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5116);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S28')) {
							if(Sp8)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 5110);
						}
						if(Sp8)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5114);
						if(Sp8)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5111);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W26S28')) {
						...
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W26S28')) {
							if(Sp8)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 5130);
						}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W26S27')) {
						...
						if(Sp11)	spawns.tryCreateCreep(spawn, UPGRADER[L], 5095);
					}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W25S27')) {
						...
						if(Sp10)	spawns.tryCreateCreep(spawn,    UPGRADER[L], 5205);
					}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W26S26') &&
						...
						if(tools.getInviderCoreLevel('W26S26') == 1) {
							if(Sp8 || Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[9][M], 5287);
						}
					}
					else {
						if(Sp8 || Sp11)	spawns.tryCreateCreep(spawn, ATTACKER[9][L], 5286);
						if(Sp8 || Sp11)	spawns.tryCreateCreep(spawn, WORKER[7][S+1], 5284);
						if(Sp8 || Sp11)	spawns.tryCreateCreep(spawn,   CARIER[7][L], 5281);
					}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S26')) {
						...
						if(Sp12)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5141);
						if(Sp12)	spawns.tryCreateCreep(spawn, UPGRADER[L], 5145);
					}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S25')) {
						if(Sp12)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5306);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S25')) {
							if(Sp12)	spawns.tryCreateCreep(spawn, CLAIMER[7][M], 5300);
						}
						if(Sp12)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5304);
						if(Sp12)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5301);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S24')) {
						if(Sp12)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5346);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S24')) {
							if(Sp12)	spawns.tryCreateCreep(spawn, CLAIMER[7][M], 5340);
						}
						if(Sp12)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5344);
						if(Sp12)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5341);
					}
        }
          ...
          if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S51')) {
            ...
            if(Sp2)	spawns.tryCreateCreep(spawn, CARIER[7][L], 491); // 2
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W58S53')) {
						if(Sp1)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 506);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W58S53')) {
              if(Sp1)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 500);
						}
						if(Sp1)	spawns.tryCreateCreep(spawn, WORKER[5][M], 504);
            if(Sp1)	spawns.tryCreateCreep(spawn, CARIER[7][L], 501);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W58S54')) {
						...
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W58S54')) {
              //if(Sp1)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 510);
						}
						//if(Sp1)	spawns.tryCreateCreep(spawn, WORKER[5][M], 514);
            ...
            if(Sp5)	spawns.tryCreateCreep(spawn, CARIER[7][M], 551);
						...
						if(Sp5)	spawns.tryCreateCreep(spawn, CARIER[7][L], 552);
					}
            ...
            if(Sp5)	spawns.tryCreateCreep(spawn,  CARIER[7][L], 561);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W55S53')) {
						if(Sp7)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 576);
					}
					else {
						if(Sp7)	spawns.tryCreateCreep(spawn, WORKER[7][L], 574);
            if(Sp7)	spawns.tryCreateCreep(spawn, CARIER[7][L], 571);
					}
            ...
            if(Sp7)	spawns.tryCreateCreep(spawn, CARIER[7][H], 591);
              ...
              if(Sp4)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 600);
						}
						if(Sp4)	spawns.tryCreateCreep(spawn, WORKER[5][M], 604);
            if(Sp4)	spawns.tryCreateCreep(spawn, CARIER[7][L], 601);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W54S53')) {
						if(Sp4)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 616);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W54S53')) {
              if(Sp4)	spawns.tryCreateCreep(spawn, CLAIMER[7][H], 610);
						}
						if(Sp4)	spawns.tryCreateCreep(spawn, WORKER[5][M], 614);
            if(Sp4)	spawns.tryCreateCreep(spawn, CARIER[7][L], 611);
					}
