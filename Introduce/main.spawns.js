    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && spawn.name != 'Spawn19' && Nspawns) {
				...
				if(Memory.totals.CARRY < 9) spawns.tryCreateCreep(spawn,          506, 10, 2); // E  550 Carier
				if(Memory.totals.CARRY < 9) spawns.tryCreateCreep(spawn,          303, 10, 3); // E  300 Carier
				if(Memory.totals.WORK < 16) spawns.tryCreateCreep(spawn,        80808, 20, 1); // E 1600 Worker
				...
				if(false) {
					...
					if(Game.cpu.bucket > constants.CPU_BUCKET_TO_SPAWN_CLAIMING_ROOMS && Memory.totals.WORK > 100) {
					...
					if(Memory.spawn_to_attack && Memory.totals.WORK > 100) {
