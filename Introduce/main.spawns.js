...
var spawns = {
   ...
   work_efficiency: function(type,range) {
    ...
    tryCreateCreep: function(spawn, type, weight, needed = 0) {
			...
			Memory.totals.NeedsCreeps = Memory.CreepsNeedsByWeight.map(function(o){return o.needs}).reduce(function(s, c) {return s + c;}, 0);
			Memory.totals.NeedsPlusCreeps = Memory.CreepsNeedsByWeight.map(function(o){return o.needs_plus}).reduce(function(s, c) {return s + c;}, 0);
...
/*
	        ...
	        spawns.tryCreateCreep(spawn,             3015, 61, 2); // V 1-1 E    Carier
					spawns.tryCreateCreep(spawn,            90909, 69, 3); // V 1-1 E    Worker
          //                                                21
          ...
          spawns.tryCreateCreep(spawn,  500100000000015, 68, 1); // V 1-2 E  RAttacker
					...
					spawns.tryCreateCreep(spawn,  500100000000015, 76, 3); // V 1-2 E  RAttacker
					spawns.tryCreateCreep(spawn,  200000008000005, 77, 3); // V 1-2 E     Healer
          //                                                 7
					spawns.tryCreateCreep(spawn,           130911, 79, 1); // V 1-1 E    Worker
          spawns.tryCreateCreep(spawn,             3015, 71, 1); // V 1-1 E    Carier
   				//																					 +2 = 23
					spawns.tryCreateCreep(spawn,  200000008000005, 99, 2); // V 1-2 E     Healer
// 					spawns.tryCreateCreep(spawn,  500100000000015,100, 3); // V 1-2 E  RAttacker
				}
