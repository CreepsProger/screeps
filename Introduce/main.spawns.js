		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && Nspawns) {
				...
				const Sp14= (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):
				         		(Game.shard.name == 'shard1')? (All || spawn.name == 'Spawn39' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):
				            (Game.shard.name == 'shard3')? (All || spawn.name == 'Spawn--' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):All;
        ...
        if(Game.shard.name == 'shard1') {
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S25')) {
						...
						if(Sp13)	spawns.tryCreateCreep(spawn, UPGRADER[H], 5305);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S24')) {
						if(Sp13)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5346);
					}
					else {
						if(Sp13)	spawns.tryCreateCreep(spawn,ATTACKER[5][M], 5346);
						if(Sp13)	spawns.tryCreateCreep(spawn, WORKER[7][M], 5344);
						if(Sp13)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5341);
						if(Sp13)	spawns.tryCreateCreep(spawn,  UPGRADER[M], 5345);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S24')) {
						if(Sp13)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5356);
					}
					else {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S23')) {
						if(Sp13)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5366);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S23')) {
							if(Sp13)	spawns.tryCreateCreep(spawn, CLAIMER[7][M], 5360);
						}
						if(Sp13)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5364);
						if(Sp13)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5361);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S23')) {
						if(Sp13)	spawns.tryCreateCreep(spawn, ATTACKER[5][M], 5376);
					}
					else {
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W28S23')) {
							if(Sp13)	spawns.tryCreateCreep(spawn, CLAIMER[7][M], 5370);
						}
						if(Sp13)	spawns.tryCreateCreep(spawn, WORKER[5][M], 5374);
						if(Sp13)	spawns.tryCreateCreep(spawn, CARIER[7][L], 5371);
					}
        }
          ...
          if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S51')) {
            ...
            if(Sp8)	spawns.tryCreateCreep(spawn,  CARIER[7][L], 631);
					...
					if(Sp2 && !All && upgrade) spawns.tryCreateCreep(spawn,   CARIER[8][L], 63);
					if(Sp2 && !All && upgrade) spawns.tryCreateCreep(spawn,   CARIER[8][L], 173);
