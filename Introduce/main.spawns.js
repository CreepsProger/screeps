	...
	run: function() {
		...
		for(var name in Game.spawns) {
			...
			if(!spawn.spawning) {
				...
				if(CL >= 4) spawns.tryCreateCreep(spawn,        90909, 40, N<(config.maxCreepsNumber+1)? 7:0); // V 1-1 E  1800   Worker
				if(CL >= 4) spawns.tryCreateCreep(spawn,        90909, 60, N<(config.maxCreepsNumber+1)? 3:0); // V 1-1 E  1800   Worker
