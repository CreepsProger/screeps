        ...
        const CARIER = [ [     0,      0,      0]  // 0  000
				...
				if(false) {
					...
					if(conditions.TO_SPAWN_MAIN_ROOMS()) {
						...
						if(Sp2	)	spawns.tryCreateCreep(spawn, CARIER[6][H], 41, 1, 1+X);
            ...
            if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S35')) {
              if(Sp4) spawns.tryCreateCreep(spawn, ATTACKER[M], 125, 1, 1);
            }
            else {
              if(Sp234) spawns.tryCreateCreep(spawn, WORKER[7][H], 124, 1, 1+X+X+X+X);
              ...
              if(Sp4) spawns.tryCreateCreep(spawn, CARIER[6][H], 141, 1, 1);
							...
							if(conditions.TO_SPAWN_CLAIMING_ROOMS3()) {
								if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S37')) {
									...
									//if(Sp4) spawns.tryCreateCreep(spawn, CARIER[4][H], 151, 1, 1);
