    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && spawn.name != 'Spawn19' && Nspawns) {
				...
				if(false) {
					...
					if(conditions.TO_SPAWN_MAIN_ROOMS()) {
						...
						if(Sp12	)	spawns.tryCreateCreep(spawn, WORKER[3][M], 44, 1, extra_upgrade?3:1);
						if(Sp12	)	spawns.tryCreateCreep(spawn,         3216, 41, 1, 1); // Carier
						if(Sp234)	spawns.tryCreateCreep(spawn, WORKER[3][H], 64, 1, 1);
            ...
            if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S35')) {
              ...
              if(Sp34) spawns.tryCreateCreep(spawn, WORKER[2][H],124, 1, 2); // Worker
              if(Sp34) spawns.tryCreateCreep(spawn,         1608,121, 1, 2); // Carier
            }
              ...
              if(Sp123)	spawns.tryCreateCreep(spawn,      CLAIMER, 50, 1, 1);
					...
					if(conditions.TO_SPAWN_CLAIMING_ROOMS()) {
						if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S33')) {
							...
							if(Sp234) spawns.tryCreateCreep(spawn,    70506, 104, 1, 1); // Worker
