    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && spawn.name != 'Spawn19' && Nspawns) {
				...
				if(Memory.totals.CARRY < 75) spawns.tryCreateCreep(spawn,         808, 10, 2); // E  800 Carier
				if(Memory.totals.CARRY < 75) spawns.tryCreateCreep(spawn,         505, 10, 2); // E  500 Carier
				...
				if(false) {
					...
					if(conditions.TO_SPAWN_MAIN_ROOMS()) {
						if(spawn.name == 'Spawn1' || !!rerun) spawns.tryCreateCreep(spawn,           130911, 34, 1, 1); // V 1-1 E    Worker
						if(spawn.name == 'Spawn1' || !!rerun) spawns.tryCreateCreep(spawn,             3015, 31, 1, 1); // V 1-1 E    Carier
						if(spawn.name == 'Spawn2' || !!rerun) spawns.tryCreateCreep(spawn,           140711, 44, 1, 1); // V 1-1 E    Worker
						if(spawn.name == 'Spawn2' || !!rerun) spawns.tryCreateCreep(spawn,             3015, 41, 1, 1); // V 1-1 E    Carier
						if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,           140711, 64, 1, 1); // V 1-1 E    Worker
					...
					if(conditions.TO_SPAWN_CLAIMING_ROOMS()) {
						if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S33')) {
							...
							if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,             3015, 91, 1, 1); // V 1-1 E    Carier
						}
							...
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S35')) {
								...
								if(conditions.TO_SPAWN_CLAIMING_ROOMS2()) {
									if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S35')) {
										...
										if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,  500000007000006,127, 1, 1); // V 1-1 E    Healer
									}
									else {
