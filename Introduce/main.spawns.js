    ...
    const I = Game.time % constants.TICKS_TO_SPAWN;
		...
		// for(var name in Game.spawns)
     {
			...
			if(!spawn.spawning && Nspawns) {
				...
				const Sp4 = (Game.shard.name == 'shard0')? (All || spawn.name == 'Spawn10' || spawn.name == 'Spawn--' || spawn.name == 'Spawn--'):
        ...
        if(Game.shard.name == 'shard1') {
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W25S28')) {
						...
						if(Sp8)	spawns.tryCreateCreep(spawn, UPGRADER[L], 5275);
						if(Sp8)	spawns.tryCreateCreep(spawn, WORKER[7][L], 5274);
						if(Sp9)	spawns.tryCreateCreep(spawn, CARIER[5][H], 5271);
					}
          ...
          if(conditions.TO_SPAWN_ROOM_DEFENDERS('W58S51')) {
            ...
            if(Sp2)	spawns.tryCreateCreep(spawn, CARIER[7][L], 481);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S52')) {
						...
						if(Sp4)	spawns.tryCreateCreep(spawn, CARIER[6][H], 441);
            ...
            if(Sp4)	spawns.tryCreateCreep(spawn,  CARIER[6][H], 551);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W56S54')) {
						if(Sp1)	spawns.tryCreateCreep(spawn, ATTACKER[8][L], 566);
					}
					else {
						if(Sp1)	spawns.tryCreateCreep(spawn,  WORKER[7][L], 564);
            if(Sp4)	spawns.tryCreateCreep(spawn,  CARIER[6][L], 561);
					}
					...
					if(false) {
						...
						if(conditions.TO_SPAWN_MAIN_ROOMS()) {
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W25S33')) {
								...
								if(Sp1)	spawns.tryCreateCreep(spawn, UPGRADER[L], 35);
							}
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W26S33')) {
								...
								if(Sp2)	spawns.tryCreateCreep(spawn, UPGRADER[L], 45);
							}
							...
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S33')) {
								...
								if(Sp3)	spawns.tryCreateCreep(spawn, UPGRADER[L], 65);
							}
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S35')) {
								...
								if(Sp4) spawns.tryCreateCreep(spawn, UPGRADER[L], 125);
							}
...
// 								if(spawn.name == 'Spawn13') spawns.tryCreateCreep(spawn, DEFENDER[12][D], 176);
							...
							if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S34')) {
								...
								if(Sp6) spawns.tryCreateCreep(spawn,   UPGRADER[L], 85);
							}
