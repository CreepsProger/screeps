			...
			if(!spawn.spawning && spawn.name != 'Spawn19' && Nspawns) {
				...
				if(false) {
					...
					if(conditions.TO_SPAWN_MAIN_ROOMS()) {
						if(spawn.name == 'Spawn1' || !!rerun) spawns.tryCreateCreep(spawn,           160608, 34, 1, 1); // V 1-1 E    Worker
						if(spawn.name == 'Spawn1' || !!rerun) spawns.tryCreateCreep(spawn,             1608, 31, 1, 1); // V 1-1 E    Carier
						if(spawn.name == 'Spawn1' || spawn.name == 'Spawn2' || !!rerun)
                                                  spawns.tryCreateCreep(spawn,           130911, 44, 1, 3); // V 1-1 E    Worker
						if(spawn.name == 'Spawn1' || spawn.name == 'Spawn2' || !!rerun)
						...
						if(spawn.name == 'Spawn3' || !!rerun) spawns.tryCreateCreep(spawn,           160608, 64, 1, 3); // V 1-1 E    Worker
