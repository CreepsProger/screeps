		...
		// for(var name in Game.spawns)
     {
        ...
        if(Game.shard.name == 'shard1') {
  				...
  				if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S29')) {
  					if(Sp1)	spawns.tryCreateCreep(spawn, ATTACKER[4][M], 5015, 1, 1);
  				}
  				else {
						const m = 0+!!flags.flags['-5011'];
						const p = 0+!!flags.flags['5011'];
						if(Sp1) spawns.tryCreateCreep(spawn, WORKER[5][M], 5014, 4, 4);
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S29')) {
						...
						if(Sp12)	spawns.tryCreateCreep(spawn, WORKER[4][H], 5024, 4, 4);
						if(Sp2)	spawns.tryCreateCreep(spawn, CARIER[4][H], 5021, 2-m+p, 2-m+p);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W29S28')) {
						...
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W29S28')) {
							if(Sp1)	spawns.tryCreateCreep(spawn, CLAIMER[5][H], 5030, 1, 1);
						}
						const m = 0+!!flags.flags['-5031'];
						const p = 0+!!flags.flags['5031'];
						if(Sp1)	spawns.tryCreateCreep(spawn, WORKER[5][L], 5034, 1, 1);
						if(Sp1)	spawns.tryCreateCreep(spawn, CARIER[5][L], 5031, 3-m+p, 3-m+p);
					}
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W28S28')) {
						...
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W28S28')) {
							if(Sp1)	spawns.tryCreateCreep(spawn, CLAIMER[5][H], 5040, 1, 1);
						}
					...
					if(conditions.TO_SPAWN_ROOM_DEFENDERS('W27S29')) {
						...
						if(conditions.TO_SPAWN_ROOM_CLAIMER('W27S29')) {
							if(Sp1)	spawns.tryCreateCreep(spawn, CLAIMER[5][H], 5050, 1, 1);
						}
						const n = 0+!!flags.flags['-5054'];
						const r = 0+!!flags.flags['5054'];
						if(Sp2)	spawns.tryCreateCreep(spawn, WORKER[4][M], 5054, 2-n+r, 2-n+r);
						const m = 0+!!flags.flags['-5051'];
						const p = 0+!!flags.flags['5051'];
						if(Sp2)	spawns.tryCreateCreep(spawn, CARIER[4][L], 5051, 3-m+p, 3-m+p);
					}
        }
