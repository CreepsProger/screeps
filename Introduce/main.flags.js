...
//				,    DP2: Game.flags['DP2']			use W19S31.DP2 // defence point 2
				...
				,     NB: Game.flags['NB']			// don't build
			 	,    NB1: Game.flags['NB1']			// don't build only in range 11-DR1.color
			 	,    NB2: Game.flags['NB2']			// don't build only in range 11-DR1.color
   ...
   work_efficiency: function(type,range) {
	...
	Limits: function(Limits) {
		...
		if(Game.shard.name == 'shard1' && Limits.color == 9) {
			Game.cpu.setShardLimits({shard0:10,shard1:180,shard2:15,shard3:15});
		}
		if(Game.shard.name == 'shard0' && Limits.color == 4) {
			Game.cpu.setShardLimits({shard0:175,shard1:5,shard2:5,shard3:5});;
		}
