...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 4) err = tryCreateCreep(err,  61202, N<4?1:0, 59); // E 1300   Miner
         if(CL >= 4) err = tryCreateCreep(err,    620, N<4?1:0, 54); // E 1300 Carrier
         if(CL >= 4) err = tryCreateCreep(err, 110202, N<4?1:0, 50); // E 1300  Worker
         if(CL >= 4) err = tryCreateCreep(err,  31901, N<4?0:0, 59); // E 1300   Miner
         if(CL >= 4) err = tryCreateCreep(err,   2006, N<4?1:0, 55); // E 1300 Carrier
