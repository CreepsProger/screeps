...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 4) mainFlags.tryCreateCreep(err, 5000100000006, N<(maxCreepsNumber+1)?2:0, 50); // V 1-1 E 500   Attaker
         if(CL >= 4) mainFlags.tryCreateCreep(err,   20000000002, N<(maxCreepsNumber+1)?2:0, 50); // V 1-1 E 1300  Claimer
