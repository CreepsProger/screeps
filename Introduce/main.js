...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 4) err = tryCreateCreep(err,   61202, N<5?1:0, 59); // E 1300   Miner
         if(CL >= 4) err = tryCreateCreep(err, 2001012, N<5?1:0, 53); // E 1300 Carrier
         if(CL >= 4) err = tryCreateCreep(err,  100402, N<5?1:0, 50); // E 1300  Worker
         if(CL >= 4) err = tryCreateCreep(err,   22002, N<5?0:0, 58); // E 1300   Miner
         if(CL >= 4) err = tryCreateCreep(err, 2001408, N<5?1:0, 55); // E 1300 Carrier
