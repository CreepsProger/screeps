...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 5) mainFlags.tryCreateCreep(         2412, 39, N<(maxCreepsNumber+1)? 2:0); // V 1-1 E  1750   Worker
         if(CL >= 5) mainFlags.tryCreateCreep(         2412, 37, N<(maxCreepsNumber+1)? 2:0); // V 1-1 E  1750   Worker
         if(CL >= 5) mainFlags.tryCreateCreep(         2412, 35, N<(maxCreepsNumber+1)? 2:0); // V 1-1 E  1750   Worker
