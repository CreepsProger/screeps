...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 2) mainFlags.tryCreateCreep(        30203, 40, Memory.totals.WORK< 3? 1:0); // E 550 Worker

         if(CL >= 1) mainFlags.tryCreateCreep(        20101, 40, Memory.totals.WORK< 2? 1:0); // E 300 Worker
      }
   }
