         ...
         target = tower.pos.findClosestByRange(FIND_STRUCTURES, {
            filter: (structure) => { 
               if(structure.structureType == STRUCTURE_WALL) {
                  return structure.hits < 16000;// 8000 E = 10 * 8000 / 800 = 100
               }
...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 4) mainFlags.tryCreateCreep(err,     70804, N<(maxCreepsNumber+1)?2:0, 58); // V 1-2 E 1300 Harvester
         if(CL >= 4) mainFlags.tryCreateCreep(err,     70804, N<(maxCreepsNumber+1)?0:0, 58); // V 1-2 E 1300    Worker
