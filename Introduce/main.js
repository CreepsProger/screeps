...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 5) mainFlags.tryCreateCreep(        31811, 40, N<(maxCreepsNumber+1)? 0:0); // V 1-1 E  1750   Worker
         if(CL >= 5) mainFlags.tryCreateCreep(         2412, 30, N<(maxCreepsNumber+1)? 6:0); // V 1-1 E  1750   Worker
