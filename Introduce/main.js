      ...
      var closestDamagedStructure = tower.pos.findClosestByRange(FIND_STRUCTURES, {
         filter: (structure) => { 
            if(structure.structureType == STRUCTURE_WALL) {
               return structure.hits < 8000;// 8000 E = 10 * 8000 / 800 = 100
            }
            if(structure.structureType == STRUCTURE_RAMPART) {
               return structure.hits < 80000;// 8000 E = 10 * 8000 / 800 = 100
            }
...
//      if(((Memory.totals.CreepsNumber < 8) || (2 * Memory.totals.FreeCapacity <=  Memory.totals.UsedCapacity)) && !Spawn.spawning) {
      if(!Spawn.spawning) {
         ...
         if(CL >= 4) mainFlags.tryCreateCreep(err, 200000911, N<6?1:0, 53); // E 1300 Carrier
         if(CL >= 4) mainFlags.tryCreateCreep(err,     60707, N<6?1:0, 50); // E 1300     Avg
         if(CL >= 4) mainFlags.tryCreateCreep(err,     61103, N<6?1:0, 59); // E 1300   Miner
         if(CL >= 4) mainFlags.tryCreateCreep(err,    110202, N<6?1:0, 51); // E 1300  Worker
         if(CL >= 4) mainFlags.tryCreateCreep(err, 200001208, N<6?1:0, 55); // E 1300 Carrier
